 ...
 *
 * ## db/init:
 *  Initializes PostgreSQL 'insurance' database and 'insurance' role for
...
class DbController extends Controller
{
    ...
    private function migrate()
    {
        $db = \Yii::$app->db;
        $db->open();
        $migrations = array_diff(scandir($dir = \Yii::getAlias('@app') . '/commands/database'), ['.', '..', '_migration.sql']);
    ...
    {
        $file = fopen(\Yii::getAlias('@app') . '/commands/database/_migration.sql', 'w+');
    ...
    {
        $this->outLine();
        $message = (new \ReflectionObject($e))->getShortName() . '. ' . $e->getMessage();
